...
template <int mmq_x>
static void instantiate_mmq_q4_k(float *tmp_fixup, const mmq_args &args,
  ...
  const int nbytes_shared = mmq_get_nbytes_shared<GGML_TYPE_Q4_K>(
...
static void launch_mmq_case_q4_k(float *tmp_fixup, const mmq_args &args,
                                 cudaStream_t stream, int cc, int nsm,
                                 size_t smpbo, int warp_size_host) {
  ...
  for (int mmq_x = 8; mmq_x <= mmq_x_max && ntiles_x_best > 1; mmq_x += 8) {
    ...
    if (mmq_x % granularity != 0)
      continue;
    const size_t nbs = mmq_get_nbytes_shared<GGML_TYPE_Q4_K>(
        mmq_x, mmq_y, cc, warp_size_host, nwarps);
    if (nbs > smpbo)
      continue;
  ...
  switch (mmq_x_best) {
  case 8:
    instantiate_mmq_q4_k<8>(tmp_fixup, args, stream, cc, nsm, smpbo,
                            warp_size_host);
    break;
  case 16:
    instantiate_mmq_q4_k<16>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 24:
    instantiate_mmq_q4_k<24>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 32:
    instantiate_mmq_q4_k<32>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 40:
    instantiate_mmq_q4_k<40>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 48:
    instantiate_mmq_q4_k<48>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 56:
    instantiate_mmq_q4_k<56>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 64:
    instantiate_mmq_q4_k<64>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 72:
    instantiate_mmq_q4_k<72>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 80:
    instantiate_mmq_q4_k<80>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 88:
    instantiate_mmq_q4_k<88>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 96:
    instantiate_mmq_q4_k<96>(tmp_fixup, args, stream, cc, nsm, smpbo,
                             warp_size_host);
    break;
  case 104:
    instantiate_mmq_q4_k<104>(tmp_fixup, args, stream, cc, nsm, smpbo,
                              warp_size_host);
    break;
  case 112:
    instantiate_mmq_q4_k<112>(tmp_fixup, args, stream, cc, nsm, smpbo,
                              warp_size_host);
    break;
  case 120:
    instantiate_mmq_q4_k<120>(tmp_fixup, args, stream, cc, nsm, smpbo,
                              warp_size_host);
    break;
  case 128:
    instantiate_mmq_q4_k<128>(tmp_fixup, args, stream, cc, nsm, smpbo,
                              warp_size_host);
    break;
  default:
    break;
...
extern "C" void launch_mmq_gguf_q4_k(void *tmp_fixup_ptr, const void *x,
  ...
  launch_mmq_case_q4_k((float *)tmp_fixup_ptr, args, (cudaStream_t)stream, cc,
                       nsm, smpbo, warp_size_host);
}

DEFINE_MMQ_MOE_LAUNCHER(q4_k, GGML_TYPE_Q4_K, launch_mmq_case_q4_k)
