...
| MTP | Qwen3-Next (Qwen3.6-35B MoE) | Built into the checkpoint (`--mtp`) | Supported with paged attention |
...
## Qwen3-Next (Qwen3.6-35B MoE)

Qwen3-Next GGUF checkpoints carrying a next-step head work the same way: pass `--mtp` and the
loader maps the `blk.{N}.nextn.*` tensors onto the built-in `mtp.*` head, which shares the target's
embeddings and `lm_head`:

```bash
mistralrs serve -m ./Qwen3.6-35B-A3B-UD-Q4_K_XL.gguf --mtp
```

`--mtp-n-predict` defaults to 2 (3 when the target hidden size is 4096 or more). The same
exactness guarantee applies: greedy output matches non-MTP decoding up to floating-point
tie-breaks, and rejected speculative steps roll the hybrid recurrent state back by replaying only
the accepted prefix.
...
`MISTRALRS_DFLASH_ADAPTIVE=0` (or `false`) to disable load adaptation. The drafter's projection
