...
| `MISTRALRS_MANAGED_WEIGHTS` | `MISTRALRS_MANAGED_WEIGHTS=1` allocates GGUF weight tensors in HIP managed memory on ROCm, filling them from the host and prefetching to the device instead of a host-to-device copy. Composes with `MISTRALRS_GGUF_NO_MMAP` and `MISTRALRS_GGUF_DROP_HOST_AFTER_LOAD`. No effect on other backends. |
